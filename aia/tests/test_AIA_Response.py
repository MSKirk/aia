from AIA_Response import AIAEffectiveArea, time_interpolate
import warnings
import numpy as np
import pandas as pd
...
def test_aia_eff_area(test_target_time, test_wavelength):
    ...
    output = aia_effective_area.effective_area(test_wavelength, test_target_time)
    assert output[0] < 1.

    try:
        warnings.simplefilter("ignore")
        output = aia_effective_area.effective_area(test_wavelength, '2000-01-01')
    except Warning:
        assert True
...
def test_time_interpolate(test_target_time):
    ...
    output = time_interpolate(ts, test_target_time)
